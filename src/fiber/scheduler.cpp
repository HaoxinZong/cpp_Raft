...
void Scheduler::start() {
  ...
  // thread类的构造函数中的run函数会执行 scheduler的run， 线程池中的线程都有一个线程级调度器
  ...
  while (true) {
    ...
    bool tickle_me = false;
    {
      ...
      while (it != tasks_.end()) {
        ...
        // thread为指定该任务运行的线程号，若为-1 ，则表示任意线程
...
void Scheduler::stop() {
  ...
  { 
    // 在锁中安全地交换threadpool_中的指针到threads中
    Mutex::Lock lock(mutex_);
