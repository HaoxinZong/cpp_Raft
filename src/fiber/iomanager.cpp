...
IOManager::IOManager(size_t threads, bool use_caller, const std::string &name) : Scheduler(threads, use_caller, name) {
  ...
  event.events = EPOLLIN | EPOLLET; // 设置读事件以及边缘触发
  ...
  // 启动scheduler，开始进行协程调度，初始化了线程池 ， 执行run函数
  start();
}
IOManager::~IOManager() {
  stop();
...
// 添加事件 fd描述符发生event事件时执行cb函数 ，
// 这是一次性的，即触发过此事件以后，这次注册的事件就失效了，下次要触发必须再注册一次
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
  ...
  // 将fdcontext 保存在epollevent中的data.ptr中
...
// 通知调度器有任务到来
// 写pipe让idle协程从epoll_wait退出，因为有了读事件，那么idle协程yield
// 到调度协程以后 ，run函数就可以调度其他任务队列中的任务了
...
void IOManager::idle() {
  ...
  while (true) {
    ...
    int ret = 0;
    do {
      // 默认超时时间5s ，如果下一个定时器的超时时间大于5s，
      // 仍然以5s来计算超时，避免定时器超时时间太大时，epoll_wait一直阻塞
