        ...
        for (int i = 0; i < args->entries_size(); i++) {
            ...
            if (log.logindex() > getLastLogIndex()) {
                ...
                if (m_logs[getSlicesIndexFromLogIndex(log.logindex())].logterm() == log.logterm() &&
                    ...
                    myAssert(false,
                             format(
                                 "[func-AppendEntries-rf{%d}] 两节点logIndex{%d}和term{%d}相同，但是其command{%d:%d}    {%d:%d}却不同！！\n",
...
void Raft::applierTicker() { //对应GET ，APPEND , PUT操作中的 LockQueue中的timeoutpop ，定期commit 未应用的日志条目到applyChan中
    while (true) {
        m_mtx.lock();
        if (m_status == Leader) {
        ...
        auto applyMsgs = getApplyLogs(); // 获取已提交但尚未应用到状态机的日志条目
        
        
        if (!applyMsgs.empty()) 
        {
            DPrintf("[func- Raft::applierTicker()-raft{%d}] 向kvserver Report的applyMsgs长度为：{%d}", m_me,
                    applyMsgs.size());
            m_mtx.unlock();
            for (auto &message: applyMsgs) 
            {
                    applyChan->Push(message); //将它推进applyChan中
            }
        }
        else
        {
...
void Raft::doElection() {
    ...
    if (m_status != Leader) {
        ...
        for (int i = 0; i < m_peers.size(); i++) {
            ...
            std::shared_ptr<raftRpcProctoc::RequestVoteArgs> requestVoteArgs = std::make_shared<
                raftRpcProctoc::RequestVoteArgs>();
            requestVoteArgs->set_term(m_currentTerm);
            requestVoteArgs->set_candidateid(m_me);
            requestVoteArgs->set_lastlogindex(lastLogIndex);
            requestVoteArgs->set_lastlogterm(lastLogTerm);
            std::shared_ptr<raftRpcProctoc::RequestVoteReply> requestVoteReply = std::make_shared<
...
void Raft::doHeartBeat() {
    ...
    if (m_status == Leader) {
        ...
        for (int i = 0; i < m_peers.size(); i++) {
            ...
            std::shared_ptr<raftRpcProctoc::AppendEntriesArgs> appendEntriesArgs = std::make_shared<
            ...
            //构造返回值
            const std::shared_ptr<raftRpcProctoc::AppendEntriesReply> appendEntriesReply = std::make_shared<
        ...
        m_lastResetHeartBeatTime = now(); //设置本次leader发送心跳的时间
    }
}
...
void Raft::leaderHeartBeatTicker() {
    while (true) {
        ...
        auto suitableSleepTime = std::chrono::milliseconds(HeartBeatTimeout) + m_lastResetHeartBeatTime - nowTime;
        m_mtx.unlock();
        if (suitableSleepTime.count() < 1) {
...
bool Raft::matchLog(int logIndex, int logTerm) {
    myAssert(logIndex >= m_lastSnapshotIncludeIndex && logIndex <= getLastLogIndex(),
             format("接收到的index不合法 :logIndex{%d}>=rf.lastSnapshotIncludeIndex{%d}&&logIndex{%d}<=rf.getLastLogIndex{%d}",
...
void Raft::RequestVote(const raftRpcProctoc::RequestVoteArgs *args, raftRpcProctoc::RequestVoteReply *reply) {
    ...
    //只有没投票，且candidate的日志的新的程度 ≥ 接受者的日志新的程度 才会投票
...
bool Raft::sendRequestVote(int server, std::shared_ptr<raftRpcProctoc::RequestVoteArgs> args,
    ...
    DPrintf("[func-sendRequestVote rf{%d}] 向server{%d} 发送 RequestVote 完成，耗時:{%d} ms", m_me, m_currentTerm,
...
bool
Raft::sendAppendEntries(int server, std::shared_ptr<raftRpcProctoc::AppendEntriesArgs> args,
    ...
    if (!reply->success()) {
        ...
        *appendNums = *appendNums + 1;
        DPrintf("接收到AE的回复---- 节点{%d}返回true,当前*appendNums{%d}", server,
                *appendNums);
        ...
        if (*appendNums >= 1 + m_peers.size() / 2) {
            ...
            *appendNums = 0;
            ...
            if (args->entries_size() > 0 && args->entries(args->entries_size() - 1).logterm() == m_currentTerm) {
                DPrintf(
                    " 获得的回复超过半数，当前term有log成功提交，更新leader的m_commitIndex from{%d} to{%d}",
...
void Raft::StartConses(Op command, int *newLogIndex, int *newLogTerm, bool *isLeader) {
    std::lock_guard<std::mutex> lg1(m_mtx);
    //    m_mtx.lock();
    //    Defer ec1([this]()->void {
    //       m_mtx.unlock();
    //    });
    if (m_status != Leader) { //若不是leader ，直接退出
    ...
    raftRpcProctoc::LogEntry newLogEntry;       //设置一个新的需要由领导发送到各个raft节点的logEntry
    ...
    persist();
    *newLogIndex = newLogEntry.logindex();     //设置参数中接受新建的logEntry的index
    *newLogTerm = newLogEntry.logterm();       //设置参数中接受新建的logEntry的index
    *isLeader = true;                          //只有leader能拿到新的logentry
}
