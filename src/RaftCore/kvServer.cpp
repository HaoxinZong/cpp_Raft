...
void KvServer::Get(const raftKVRpcProctoc::GetArgs *args, raftKVRpcProctoc::GetReply *reply) {
    ...
    int _ = -1;
    bool isLeader = false;
    m_raftNode->StartConses(op, &raftIndex, &_, &isLeader); //raftIndex：raft预计的logIndex ，虽然是预计，但是正确情况下是准确的，op的具体内容对raft来说 是隔离的
...
void KvServer::PutAppend(const raftKVRpcProctoc::PutAppendArgs *args, raftKVRpcProctoc::PutAppendReply *reply) {
    ...
    m_raftNode->StartConses(op, &raftIndex, &_, &isleader); //在这里 raftIndex中是为此次Op新建的logentry的 logindex
    ...
    if (!chForRaftIndex->timeOutPop(CONSENSUS_TIMEOUT, &raftCommitOp)) { //在上面若还没存在为当前logentry的index设置的LockQueue ，则新建一个，在这里判断
                                                                         //共识时间（leader发送往各raft节点，并返回超过半数节点时，会往队列中放入一个消息）
        /*
    ...
    } else {    //共识timeout时间中返回了，在集群中成功了 ，这里的命令执行成功指，本条命令在整个raft集群中达到同步状态
        DPrintf(
...
void KvServer::ReadRaftApplyCommandLoop() {
    while (true) {
        ...
        DPrintf("--tmp--[func-KvServer::ReadRaftApplyCommandLoop()-kvserver{%d}] 收到了以下raft的消息",
                m_me);
...
KvServer::KvServer(int me, int maxraftstate, std::string nodeInforFileName, short port) {
    ...
    for (int i = 0; i < ipPortVt.size(); ++i) {
        ...
        auto *rpc = new RaftRpc(otherNodeIp, otherNodePort);   ///RaftRpc维护当前节点对其他某一个结点的所有rpc发送通信的功能
        servers.push_back(std::shared_ptr<RaftRpc>(rpc));

        std::cout << "node" << m_me << " 连接node" << i << "success!" << std::endl;
    }
    sleep(ipPortVt.size() - me); //等待所有节点相互连接成功，再启动raft
    m_raftNode->init(servers, m_me, persister, applyChan);       /// Raft .init 函数启动raft节点
